document.addEventListener('DOMContentLoaded', function () {
    ...
    function initializeAll() {
        ...
        if(attachDateEl && attachTimeEl){
            ...
            } catch(e) { console.error("Error setting date/time", e); }
            attachDateEl.addEventListener('change', calculateRemovalDate);
            attachTimeEl.addEventListener('change', calculateRemovalDate);
        }
        
        // TNR 탭
        const totalWeightTNR = document.getElementById('totalWeightTNR');
        const protocolTNR = document.getElementById('protocolTNR');
        if(totalWeightTNR && protocolTNR) {
            totalWeightTNR.addEventListener('input', calculateTNRDose);
            protocolTNR.addEventListener('change', calculateTNRDose);
        }
        
        // 퇴원약 탭
        document.getElementById('prescription_days_cat').addEventListener('input', calculateAll);
    ...
    function updateAllTitles() {
        ...
        const placeholders = document.querySelectorAll('.patient-name-placeholder');
        placeholders.forEach(el => el.textContent = nameOrDefault);
    ...
    function saveActiveTabAsImage() {
        ...
        // Select the specific content wrapper inside the active tab
        const contentToCapture = activeTab.querySelector('.printable-content > div');
        if (!contentToCapture) {
            console.error("No printable content found in the active tab.");
            return;
        }
    ...
    function calculateAll() {
        ...
        if (!weightInput.value || isNaN(weight) || weight <= 0) {
            ...
            document.getElementById('pre_op_drugs_result_cat').innerHTML = '<div class="p-4 text-center text-gray-500">체중을 입력해주세요.</div>';
            document.getElementById('nerve_block_result_cat').innerHTML = '<div class="p-4 text-center text-gray-500">체중을 입력해주세요.</div>';
            document.getElementById('ketamine_cri_result_cat').innerHTML = '<div class="p-4 text-center text-gray-500">체중을 입력해주세요.</div>';
            document.getElementById('hypotension_protocol_cat').innerHTML = '<div class="p-4 text-center text-gray-500">체중을 입력해주세요.</div>';
            document.getElementById('bradycardia_protocol_cat').innerHTML = '';
            document.getElementById('cpa_protocol_cat').innerHTML = '<div class="p-4 text-center text-gray-500">체중을 입력해주세요.</div>';
            document.getElementById('discharge_cat').innerHTML = '<div class="p-4 text-center text-gray-500">체중을 입력해주세요.</div>';
    ...
    function populatePrepTab(weight) {
        ...
        // --- Anesthetic Induction Logic (MODIFIED) ---
        ...
        let inductionHtml = '';
        if (status === 'cardiac') {
            inductionHtml = `<p><span class="result-value">${alfaxanMlMin.toFixed(2)}~${alfaxanMlMax.toFixed(2)} mL</span> (${alfaxanMgMin.toFixed(1)}~${alfaxanMgMax.toFixed(1)} mg) 알팍산 <span class="text-red-600 font-bold">- 추천</span></p>`;
        } else { // healthy
            inductionHtml = `
                <p class="text-sm"><span class="result-value">${propofolMlMin.toFixed(2)}~${propofolMlMax.toFixed(2)} mL</span> (${propofolMgMin.toFixed(1)}~${propofolMgMax.toFixed(1)} mg) 프로포폴</p>
                <p class="text-sm"><span class="result-value">${alfaxanMlMin.toFixed(2)}~${alfaxanMlMax.toFixed(2)} mL</span> (${alfaxanMgMin.toFixed(1)}~${alfaxanMgMax.toFixed(1)} mg) 알팍산</p>
            `;
        }
        const chillText = isChill ? '<p class="text-xs text-red-600 font-bold mt-1">※ Chill 50% 감량</p>' : '';
        const inductionDrugHtml = `<div class="p-3 bg-indigo-50 rounded-lg"><h4 class="font-bold text-indigo-800">마취 유도제</h4>${inductionHtml}${chillText}</div>`;
        // --- End of Modification ---
        ...
        const blockSitesEl = document.getElementById('cat_block_sites');
        if (blockSitesEl) {
            blockSitesEl.value = sites;
            blockSitesEl.addEventListener('change', calculateAll);
        }
    ...
    function populateDischargeTab(weight) {
        ...
        if (renalStatus === 'healthy') {
            document.getElementById('vetrocam_days_cat')?.addEventListener('input', calculateAll);
            document.getElementById('gabapentin_dose_cat_a')?.addEventListener('input', calculateAll);
        } else {
            document.getElementById('gabapentin_dose_cat_b')?.addEventListener('input', calculateAll);
        }
    }
    ...
    function calculateTNRDose() {
        const totalWeightInput = document.getElementById('totalWeightTNR');
        const protocolSelect = document.getElementById('protocolTNR');
        const resultDiv = document.getElementById('resultTNR');
        if(!totalWeightInput || !protocolSelect || !resultDiv) return;

        const totalWeight = parseFloat(totalWeightInput.value);
        const protocol = protocolSelect.value;
        const cageWeight = 3.6; 
        ...
        switch (protocol) {
            case 'dkb': doseRate = 0.1; protocolName = 'DKB'; break;
            case 'dkz': doseRate = 0.05; protocolName = 'DKZ'; break;
        }

        const finalDose = catWeight * doseRate;

        resultDiv.innerHTML = `고양이 예상 체중: <strong>${catWeight.toFixed(2)} kg</strong><br>권장 주사 용량(${protocolName}): <strong>${finalDose.toFixed(2)} mL</strong>`;
